...
class Logger:
    """로그를 파일과 콘솔에 동시에 출력"""
    def __init__(self, log_file='process_log_final.txt'):
        self.log_file = log_file
        self.start_time = time.time()
        with open(log_file, 'w', encoding='utf-8') as f:
            f.write("="*60 + "\n")
            f.write("협력사 중복 탐지 및 병합 프로세스 로그 (최종 최적화 버전)\n")
            f.write("="*60 + "\n\n")
...
print("="*60)
print("협력사 중복 탐지 및 병합 - 최종 최적화 버전")
print("="*60)
print("\n핵심 기능:")
print("  ✓ 다국어 중복 탐지: 같은 업체의 다른 언어 표기도 탐지")
print("    예: '阿里巴巴集团' ↔ 'Alibaba Group'")
print("  ✓ 다중 Blocking 키: 비교 횟수 대폭 감소")
print("  ✓ RapidFuzz: 빠른 문자열 비교")
print("  ✓ 2단계 접근법: 후보 추출 → 정교한 비교")
print("="*60)
...
# 중복 탐지
logger.section("2단계: 다국어 중복 탐지 (최적화 방식)")
logger.log("중복 탐지 시작 (유사도 임계값: 0.85)")
logger.log("다국어 중복 탐지 지원: 활성화")

detector = DuplicateDetectorFinal(similarity_threshold=0.85)
...
if duplicate_groups:
    ...
    output_file = f'bio_vendor_merged_final_{timestamp}.xlsx'
